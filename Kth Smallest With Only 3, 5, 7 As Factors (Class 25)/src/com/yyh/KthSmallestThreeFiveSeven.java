...
public class KthSmallestThreeFiveSeven {
	...
	public long kth(int K){
		...
		while (K > 1){
			...
			// for the star <x,y+1,z>, the actual value is *5
	...
	public long kthII(int K){
		...
		long result = seed;
		while (K > 1){
			// each round, pick the smallest one from the head of the three deques. when pushing back the value into the deques, following the rule:
			// if the smallest number x is from deque3:
			// we need to push x*3 to deque3, x*5 to deque5 and x*7 to deque7, to maintain the property of the three deques
			// if the smallest number x is from deque5:
			// we only need to push x*5 to deque5 and x*7 to deque7, we do not need to push x*3 again, because x*3 has to be already generated
			// before, x= 3^a*5^b, x*3=3^(a+1)*5^b=3^(a+1)*5^(b-1)*5, and 3^(a+1)*5^(b-1)<x, it means x*3 has to be already
