...
public class FourSumII {
	...
	public boolean existIII(int[] array, int target){
		...
		// the order of traversing i,j is not arbitrary, we should guarantee we can always look at the pair with the smallest right index
		for (int i = 1; i < array.length; i++){
			for (int j = 0; j < i; j++){
				int pairSum = array[j] + array[i];
				// we need to guarantee there exists another pair with right index smaller than the current pair's left index
